<?php 
class ConcertFeedController extends ControladorBase{
                ...
                $html .= '<div class="row mb-2">
                            <div class="col-md-12">
                                <div class="card flex-md-row mb-4 box-shadow h-md-250">
                                    ...
                                    <div class="mb-1 text-muted">'.$fecha_evento.' - '.$res['hora_evento'].'</div>
                                        <p class="card-text mb-auto">'.$res['descripcion_evento'].'</p>
                                    </div>
                                    <button type="button" class="btn btn-primary" onclick="ComprarBoleto('.$res['id_evento'].')">Купить Билеты</button>
                                </div>
                            </div>
                        </div>';
            }
    ...
    public function GetUserName()
    {
        ...
        $columnas="usuarios.nombre, usuarios.apellido";
        $tablas="usuarios";
        $where="usuarios.id_usuarios=".$_SESSION['id_usuarios'];
        $id="usuarios.id_usuarios";
        $resultSet=$model->getCondiciones($columnas, $tablas, $where, $id);
   
        echo $resultSet[0]['nombre']." ".$resultSet[0]['apellido'];
    }
